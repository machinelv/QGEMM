...
struct GEMM_Config {
    static constexpr size_t BLOCK_TILE_SIZE_M = BLOCK_M;   
    static constexpr size_t BLOCK_TILE_SIZE_N = BLOCK_N;
    static constexpr size_t BLOCK_TILE_SIZE_K = BLOCK_K;
    static constexpr size_t GROUP_SIZE_M      = GROUP_M;
    static constexpr size_t STAGE_NUMS        = STAGE;
};
...
template <typename T_INPUT, typename T_OUTPUT, 
            ...
            size_t GROUP_SIZE_M>
__global__ void GEMM_kernel(const T_INPUT* A, size_t ldA, const T_INPUT* B, size_t ldB, T_OUTPUT* C, size_t ldC, 
...
inline __device__ void process_data_from_shared_memory_using_wmma(
        ...
        wmma::fragment<wmma::matrix_a, WMMA_TILE_SIZE_M, WMMA_TILE_SIZE_N, WMMA_TILE_SIZE_K, T, wmma::col_major> 
        a_frag[WMMA_TILE_PER_WARP_M],
        wmma::fragment<wmma::matrix_b, WMMA_TILE_SIZE_M, WMMA_TILE_SIZE_N, WMMA_TILE_SIZE_K, T, wmma::row_major>
...
template <size_t BLOCK_TILE_SIZE_M, size_t BLOCK_TILE_SIZE_N, size_t BLOCK_TILE_SIZE_K,
            ...
            size_t WMMA_TILE_PER_WARP_M, size_t WMMA_TILE_PER_WARP_N, size_t WMMA_TILE_PER_WARP_K,
            size_t WMMA_TILE_SIZE_M, size_t WMMA_TILE_SIZE_N, size_t WMMA_TILE_SIZE_K>
inline __device__ void process_data_from_shared_memory_using_wmma<__nv_bfloat16, __nv_bfloat16, BLOCK_TILE_SIZE_M, BLOCK_TILE_SIZE_N, BLOCK_TILE_SIZE_K, WARP_TILE_SIZE_M, WARP_TILE_SIZE_N, WMMA_TILE_PER_WARP_M, WMMA_TILE_PER_WARP_N, WMMA_TILE_PER_WARP_K, WMMA_TILE_SIZE_M, WMMA_TILE_SIZE_N, WMMA_TILE_SIZE_K>(
        const __nv_bfloat16 A_T_shared_block_tile[BLOCK_TILE_SIZE_K][BLOCK_TILE_SIZE_M],
        const __nv_bfloat16 B_shared_block_tile[BLOCK_TILE_SIZE_K][BLOCK_TILE_SIZE_N],
        ...
        size_t warp_m_id, size_t warp_n_id )
{
    wmma::fragment<wmma::accumulator, WMMA_TILE_SIZE_M, WMMA_TILE_SIZE_N, WMMA_TILE_SIZE_K, float> acc_frag_fp32_local[WMMA_TILE_PER_WARP_M][WMMA_TILE_PER_WARP_N];
    ...
    for (size_t wmma_tile_m_idx{0U}; wmma_tile_m_idx < WMMA_TILE_PER_WARP_M; ++wmma_tile_m_idx) {
        ...
        for (size_t wmma_tile_n_idx{0U}; wmma_tile_n_idx < WMMA_TILE_PER_WARP_N;++wmma_tile_n_idx) {
            wmma::fill_fragment(acc_frag_fp32_local[wmma_tile_m_idx][wmma_tile_n_idx], static_cast<float>(0));
        }
    }
...
template <size_t BLOCK_TILE_SIZE_M, size_t BLOCK_TILE_SIZE_N, size_t BLOCK_TILE_SIZE_K,
          ...
          size_t WMMA_TILE_SIZE_M, size_t WMMA_TILE_SIZE_N, size_t WMMA_TILE_SIZE_K,
          size_t GROUP_SIZE_M>
__global__ void GEMM_kernel<__nv_bfloat16, __nv_bfloat16, 
    ...
    (const __nv_bfloat16* A, size_t ldA, const __nv_bfloat16* B, size_t ldB,
                       __nv_bfloat16* C, size_t ldC, size_t M, size_t N, size_t K)
{
    ...
    __shared__ __nv_bfloat16 A_T_block_tile[BLOCK_TILE_SIZE_K][BLOCK_TILE_SIZE_M];
    __shared__ __nv_bfloat16 B_block_tile[BLOCK_TILE_SIZE_K][BLOCK_TILE_SIZE_N];
    ...
    for (size_t block_id_k{0}; block_id_k < K; block_id_k += BLOCK_TILE_SIZE_K)
    {
        ...
        process_data_from_shared_memory_using_wmma<__nv_bfloat16, __nv_bfloat16, 
            BLOCK_TILE_SIZE_M, BLOCK_TILE_SIZE_N, BLOCK_TILE_SIZE_K,
            WARP_TILE_SIZE_M, WARP_TILE_SIZE_N, 
    ...
    for (size_t wmma_tile_idx_m{0U}; wmma_tile_idx_m < WMMA_TILE_PER_WARP_M; ++wmma_tile_idx_m) {
        #pragma clang loop unroll_count(WMMA_TILE_PER_WARP_N)
        for (size_t wmma_tile_idx_n{0U}; wmma_tile_idx_n < WMMA_TILE_PER_WARP_N; ++wmma_tile_idx_n) {
            wmma::fragment<accumulator, WMMA_TILE_SIZE_M, WMMA_TILE_SIZE_N, WMMA_TILE_SIZE_K, __nv_bfloat16> acc_frag;
            // todo: bf16 and fp32 cast
                       ...
                       __nv_bfloat16* C, size_t ldC, size_t M, size_t N, size_t K) 
{
    ...
    GEMM_kernel<BLOCK_TILE_SIZE_M, BLOCK_TILE_SIZE_N, BLOCK_TILE_SIZE_K, 
        ...
        <<<dim3((N + BLOCK_TILE_SIZE_N - 1) / BLOCK_TILE_SIZE_N, (M + BLOCK_TILE_SIZE_M - 1) / BLOCK_TILE_SIZE_M, 1),
           dim3(THREAD_NUM), 0>>>(A, ldA, B, ldB, C, ldC, M, N, K);
}
...
void GEMM_kernel_v1<__nv_bfloat16, __nv_bfloat16>(const __nv_bfloat16* A, size_t ldA,
                    ...
                    size_t M, size_t N, size_t K)
{
    // Implementation for __nv_bfloat16 - placeholder for now
    // TODO: Implement proper kernel once wmma types are resolved
    printf("GEMM_kernel_v1 called with BF16 types - not implemented yet\n");
}

// Explicit template instantiation
template void GEMM_kernel_v1<__nv_bfloat16, __nv_bfloat16>(const __nv_bfloat16*, size_t, const __nv_bfloat16*, size_t, __nv_bfloat16*, size_t, size_t, size_t, size_t);
