...
def generate_input(m: int, n: int, k: int, seed: int) -> input_t:
    ...
    # a = 0.5*(torch.ones((k, m), dtype=torch.bfloat16, device="cuda")).to(torch.float8_e4m3fnuz)
    # b = 0.5*(torch.ones((k, n), dtype=torch.bfloat16, device="cuda")).to(torch.float8_e4m3fnuz)
